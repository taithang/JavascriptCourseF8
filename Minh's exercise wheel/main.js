...
function spin(d) {
  ...
  vis
    ...
    .each("end", function () {
      ...
      d3.select(".slice:nth-child(" + (picked + 1) + ") path").remove();
      d3.select(".slice:nth-child(" + (picked + 1) + ") text").remove();
      removeItemWheel();
...
//Replace old spin wheel function
...
function render() {
  const htmls = data.map((data, index) => {
    return `
    <h3>${data.label}</h3>
    <button class = 'remove' onclick = 'removeName(${index})' >Delete</button>
    `;
  });
...
//Remove item on spin wheel
function removeItemWheel(i) {
  data.splice(i, 1);
}
